...
function getMediaUrl(id) {
  ...
  return name ? MEDIA_BASE + encodeKey(name) : null;
}

function getThumbnailHtml(id, alt = "") {
  const name = driveFileName(id);
  if (!name) return "";
  const url = getMediaUrl(id);
  if (isVideoName(name)) {
    return `<video src="${url}#t=0.1" preload="metadata" muted playsinline style="width:100%; height:100%; object-fit:cover; pointer-events:none; border-radius:inherit;"></video>`;
  }
  return `<img src="${url}" alt="${escapeHTML(alt)}" loading="lazy" decoding="async" style="width:100%; height:100%; object-fit:cover; pointer-events:none; border-radius:inherit;" />`;
}
...
function setStageHTML(html, externalUrl = "#", options = {}) {
  ...
  // Clear any explicit box sizing left over from a previous Drive video and
  // invalidate any in-flight aspect detection so this media uses clean sizing.
  stage.style.width = "";
  stage.style.height = "";
...
function parseAspectRatio(aspect, fallback = 16 / 9) {
...
// Reads a media file's true frame shape natively.
function detectMediaAspect(id) {
  return new Promise((resolve) => {
    const name = driveFileName(id);
    ...
    if (isVideoName(name)) {
      ...
      vid.onloadedmetadata = () => resolve(vid.videoWidth > 0 && vid.videoHeight > 0 ? vid.videoWidth / vid.videoHeight : null);
...
// The originals live on Google Drive; the same files are mirrored to R2 under
// their ORIGINAL filenames (flat, at the bucket root). We serve playback from
// R2 (reliable, zero egress) and keep Drive as an automatic fallback during the
// transition. CHANGE THIS ONE LINE if your R2 public/custom domain differs:
const MEDIA_BASE = "https://assets.ezz-tantawy.org/";

// Drive file id -> original filename on R2. Generated from Drive metadata.
const DRIVE_FILES = {
  "108WCYyFM_mW72J1c983kZp-tOUL55JGk": "0013.png",
  "10YoCiLyNowRK-WmFKyMXC85vD48_XCv2": "0011.png",
  "11IlhK3V7kDP_o4b-JiwFXUJacCJRTPnt": "Eid video.mp4",
  "12_LgsdVk2sDeJnwEDyHFyFOZL4VYSuNI": "Train to hire.mp4",
  "12jyuM1wb9siHtt8HiKgw8PgcTNSdaBIs": "Apartment visualization renders (17).jpeg",
  "12pig-Fz9oXvq5sRic9pL3hvYTaCgSz-g": "Apartment visualization renders (9).jpeg",
  "1427sCOtQmGH4g_2RVPwjf8zUuQ8lFWyJ": "0002.png",
  "14ABaigk9Cv_x68uRd-a0kU_m9KHAVBaF": "Ramadan tournment video.mp4",
  "14CiCAawsEl8H2YPhEiM1hL92nMB01gyy": "Apartment visualization renders (2).jpeg",
  "14Yj0ozJGmVu8L3xim1XRs8VZinrHa3Rm": "0004.png",
  "17M09AAN8D0vzx-_8YHT_WgctG3zGwwjj": "Final render_ Music.mp4",
  "1ABuqrFDy1Dx0BZpGa3vLZpJn42R4pPsU": "f64c7e248631751.69f5e85069d04.png",
  "1AKE7AK0okGNm8Hl5zn3OmRC57JZtbPYw": "Thumbnail.png",
  "1AV3jOoYEOVie3PxjdZIIMTkKzM4X4Eln": "Apartment visualization renders (19).jpeg",
  "1B7H3O60PpaiXEf-zlp0SLzyuN2S_Ti11": "0015.png",
  "1CiFp-YzDACT2LH732eiexABE8RUCRdg7": "Apartment visualization (3).png",
  "1CzwhF3w3ZRUEBPRk21qVL-bxhmPtr2Ao": "Apartment visualization renders (1).jpeg",
  "1DLEfY0WrLJZ2SBDya5viVfMJ6QatfFOt": "Apartment visualization renders (16).jpeg",
  "1F0XIQjgommn4BthummzK4_zU61F0XAFJ": "0016.png",
  "1F5yQU-KwJtSEF0NM0f5E9R60Av3_6Lui": "Apartment visualization renders (5).jpeg",
  "1FBJVaKJufYhEPle6uK00-5-pqXZhl7HB": "Apartment visualization renders (6).jpeg",
  "1GIEPO0VrljorriHQ1ZIja72UZDDwRQeE": "Apartment visualization renders (3).jpeg",
  "1GIjJvVtb_jJXWJW9LOc-ITWtPTHyOOp9": "0009.png",
  "1HDBBKxR1Xx53Lmb1-3tjXL3j4zVZGDxE": "0010.png",
  "1Izket4K-IikE9031e7OkEm9U5MIuoXB-": "c3bd28248631751.69f5e8506a550.png",
  "1JgNfhxucpOj22HrFz3M6XYGtsTEGoect": "Apartment visualization renders (12).jpeg",
  "1Mp1hzlLclSoSisMvUzr87OWgCL1dMwi6": "Apartment visualization (2).jpeg",
  "1N8ARrwt66TH01VS9sq_hHA2L8uguaR8Y": "Apartment visualization (4).jpeg",
  "1NsOFOR4kdavBR7MWD9kHgi_Qk5PE4mju": "Apartment visualization (2).png",
  "1OLw2dQqd1wN99OL331397whlKA-GYp_6": "Color grading sample.mp4",
  "1OMOkvGMj7SobrCYG4WM0ZJIRCDJRrVT1": "325f48248631751.69f5e850692ca.jpeg",
  "1P7EGhAvNz5mlI1Vd3PlQYx_5mc9xzagh": "Apartment visualization renders (11).jpeg",
  "1QMl_dT9HIqrA2Sf47U9eLY46YropvBFS": "0003.png",
  "1QQie38L4w3YBiA4rrIRYREBM8Fe5U17h": "AI easter video.mp4",
  "1R8n950aD5KmSwbZDc7ES66uzDdDf7UNx": "Building animation.mp4",
  "1RuuPM765g4xvRZE-weFZEeswWTY3uGMu": "Apartment visualization renders (18).jpeg",
  "1RzSCuUtBXO2ON2IWSD83xB_0dtLzHIaM": "0006.png",
  "1TckQm89sG1hWbaGzoP4w7YnIaOP9r7fx": "Apartment visualization (1).jpeg",
  "1Tp4WIi8WSkBf7W9pzCn6EeTkfrgSBcJK": "Apartment visualization renders (13).jpeg",
  "1VB0ZBtpIKCgap1cMYSs-moGb1yKa6h4Q": "0014.png",
  "1VKkgoHCOGMu1zHdH0j73_5TbAwcixZQO": "0020.png",
  "1ZnTxMSdc6jx3GkUhPu-dsXTbrpYrDF-h": "Showcase_3d.mp4",
  "1_HaPTDTBXMkl9-0-eMdDs9Nuh5Gh5W4Y": "Apartment visualization renders (14).jpeg",
  "1bSOVE3b2Bkdcpf9_Qhfj0w4wjT1_DVnt": "Apartment visualization renders (4).jpeg",
  "1dN3qYTVBrJ_Ql5JcnzMQgDPSQBQ4AeP3": "0017.png",
  "1e22eK7xuyWTQtfgdC8DidFJTOtgO9Faz": "0021.png",
  "1fDr8kTm_0kTOK-NgG7a7hI-yAUHMyfI5": "0019.png",
  "1fUcNfHJtvfkWPEW_6uQNOWvrmHejEXfl": "Apartment visualization (3).jpeg",
  "1g2ubozqUtcQbSxFpm3Z7s66EuLZZ0rM0": "Floor plans.png",
  "1h6vw-hqLDPMwuvweCutE_Ftlgb9xXkHX": "0007.png",
  "1i62QaFUR8gSWr2iFtfdn-t6Ax_jw36Jb": "Apartment visualization (5).jpeg",
  "1m1d1oxg0KFdO4EFrrYa3XoquAu3X_Tr8": "Apartment visualization (4).png",
  "1n4uPsuyumtPV6EfA0Gl2Y27mAv6X9jp6": "0008.png",
  "1nocwhes7ucVX8J54pPcgoQIklA3qf1tQ": "6cbe98248631751.69f5e85069803.jpeg",
  "1oNVXhSGbCKXKPeQnjWhQY1vBkZCUT19I": "Apartment visualization renders (7).jpeg",
  "1rtbVBWQizQNauRLdSu9mdljHUGzseO-5": "Ard elshog3an Promo_Ezzeldeen.mp4",
  "1ses_aBFl-Z9WTYRwlfxk3GPJm3JA-YU9": "0001.png",
  "1tR_PbFEacuIf4Etj02FLaqzZBOJnYU5q": "Apartment visualization renders (8).jpeg",
  "1vZBpsHpuFRtzJGKwiyB60JmP-jZozSzt": "Hannah Veneers.mp4",
  "1xXaqEtrhKlwjhi0qKKav1ABu_HHkRHNi": "0005.png",
  "1yNKs4SK-KGx6Oa-yNUaG0fq7NgorRIry": "Apartment visualization renders (10).jpeg",
  "1y_9yQoZWXAK_0P1p0DeR5msoj-Bdmn39": "Apartment visualization renders (15).jpeg",
  "1zBCzUcLDLvQRCRo5wSGdZsWvOcmHD0B9": "0012.png",
};

function driveFileName(id) { return DRIVE_FILES[id] || null; }
function encodeKey(key) { return key.split("/").map(encodeURIComponent).join("/"); }
function isVideoName(name) { return /\.(mp4|webm|ogg|ogv|mov|m4v)$/i.test(name || ""); }
function isImageName(name) { return /\.(jpe?g|png|gif|webp|avif|bmp|svg)$/i.test(name || ""); }

const mediaTypeCache = new Map();

// Fetches the headers from R2 to determine the actual content type.
function probeMediaType(id) {
  if (mediaTypeCache.has(id)) return Promise.resolve(mediaTypeCache.get(id));
  const url = getMediaUrl(id);
  if (!url) return Promise.resolve("");
  return fetch(url, { method: "HEAD" })
    .then((res) => {
      const type = (res.headers.get("Content-Type") || "").toLowerCase();
      mediaTypeCache.set(id, type);
      return type;
    })
    .catch(() => "");
}

// Sizes the stage to the media's real aspect ratio, fitting both the available
// width and the viewport height so a tall portrait video never overflows.
function fitStage() {
  if (!stageFit || !stage) return;
  const host = stage.parentElement;
  if (!host) return;
  const cs = getComputedStyle(host);
  const padX = (parseFloat(cs.paddingLeft) || 0) + (parseFloat(cs.paddingRight) || 0);
  const availW = Math.max(180, host.clientWidth - padX);
  const availH = Math.max(240, Math.round(window.innerHeight * 0.82));
  const capW = stageFit.maxWidth ? Math.min(availW, stageFit.maxWidth) : availW;
  let w = capW;
  let h = w / stageFit.ratio;
  if (h > availH) { h = availH; w = h * stageFit.ratio; }
  if (w > availW) { w = availW; h = w / stageFit.ratio; }
...
function loadMedia(id, label = "Media", options = {}) {
  const ratio = parseAspectRatio(options.aspect);
  const maxWidthMatch = /([\d.]+)px/.exec(options.maxWidth || "");
  
  // 1) Instant loading state using getThumbnailHtml (renders video poster or image)
  setStageHTML(
    getThumbnailHtml(id, label),
    getMediaUrl(id),
    {
      className: `stage-drive ${ratio < 1 ? "stage-portrait" : ""}`,
      aspect: options.aspect || "16 / 9",
      maxWidth: options.maxWidth || "100%"
    }
  );
  stageFit = { ratio, maxWidth: maxWidthMatch ? parseFloat(maxWidthMatch[1]) : null };
  const token = aspectToken;
  requestAnimationFrame(fitStage);

  // 2) Correct the box to the real frame shape.
  detectMediaAspect(id).then((realRatio) => {
    if (token !== aspectToken || !stageFit || !realRatio) return;
    stageFit.ratio = realRatio;
    stage.classList.toggle("stage-portrait", realRatio < 1);
    stage.style.setProperty("--stage-aspect", `${realRatio}`);
    fitStage();
  });

  // 3) Render the correct native element.
  const name = driveFileName(id);
  if (name && isVideoName(name)) { renderVideo(id, label); return; }
  if (name && isImageName(name)) { renderImage(id, label); return; }
  
  probeMediaType(id).then((type) => {
    if (token !== aspectToken) return;
    if (type.startsWith("video/")) renderVideo(id, label);
    else if (type.startsWith("image/")) renderImage(id, label);
  });
}

window.addEventListener("resize", () => { if (stageFit) fitStage(); });
window.addEventListener("orientationchange", () => {
  if (stageFit) setTimeout(fitStage, 250);
});
...
function showDriveSection(section, index = 0) {
  ...
  if (items[index]) loadMedia(items[index], `${itemLabel} ${index + 1}`);
      ...
      <button class="youtube-card tele-card ${i === index ? "active" : ""}" type="button" data-tele-index="${i}" aria-label="Open ${escapeHTML(item.title)}">
        <span class="youtube-thumb tele-thumb">
          ${item.mediaType === "drive" && item.id
            ? getThumbnailHtml(item.id, item.title)
...
function showMediaSection(section, index = 0) {
  ...
  if (section === "main") {
    ...
    else loadMedia(media.id, media.label, media);
    return;
  }
